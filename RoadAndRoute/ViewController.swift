...
class ViewController: UIViewController {
    ...
    var annotatinArray = [MKPointAnnotation]()
    ...
    @objc func addAdressButtonTapped(){
        alertAddAdress(title: "Добавить", placeholder: "Введите адрес") {[self](text) in setupPlacemark(adressPlace: text)
            
        }
    }
    @objc func routeButtonTapped(){
        for index in 0...annotatinArray.count - 2{
            createDirectionRequest(startCoordinate: annotatinArray[index].coordinate, destinationCoordinate: annotatinArray[index + 1].coordinate)
        }
        mapView.showAnnotations(annotatinArray, animated: true)
    }
    @objc func resetButtonTapped(){
        mapView.removeOverlays(mapView.overlays)
        mapView.removeAnnotations(mapView.annotations)
        annotatinArray = [MKPointAnnotation]()
    ...
    private func setupPlacemark(adressPlace: String){
        ...
        geocoder.geocodeAddressString(adressPlace) { [self](placemarks,error) in
            if let error = error {
                print(error)
                self.alertError(tile: "Ошибка", message: "Сервер недоступен")
                return
            }
            ...
            annotatinArray.append(annotation)
            if annotatinArray.count < 2 {
    ...
    private func createDirectionRequest(startCoordinate: CLLocationCoordinate2D, destinationCoordinate: CLLocationCoordinate2D) {
        let startLocatin = MKPlacemark(coordinate: startCoordinate)
        let destinationLocation = MKPlacemark(coordinate: destinationCoordinate)
        let request = MKDirections.Request()
        request.source = MKMapItem(placemark: startLocatin)
    ...
    let diraction = MKDirections(request: request)
        diraction.calculate {(responce, error) in
            ...
            guard let responce = responce else{
                self.alertError(tile: "Ошибка", message: "Маршрут недоступен")
                return
            }
